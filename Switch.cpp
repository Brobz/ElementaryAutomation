...
Switch::Switch(Color rest_color, Color color, Color hover_color, UIText text, Vector2f position, Vector2f size) : Button::Button(rest_color, hover_color, text, position, size){
    (*this).color = color;
}

void Switch::checkIfClicked(bool mouse_down){
    if(isHovered()){
        if(mouse_down) isClicked = !isClicked;
